...
Assuming we are in the root directory of the project:
 ...
 4. Finally, in the `Cargo.toml` project file of the library we just created (`src/shared/Cargo.toml`) add the following dependencies:
    ...
    ```toml
    [dependencies]
    fadroma = { version = "0.8.7", features = ["vk"] }
...
We then change our `new()` method to have an `auction: ContractCode` parameter that will have to be passed to us in the instantiate message. Inside the function body, before the returned `Response`, we then store the data:
...
```toml
[dependencies]
fadroma = { version = "0.8.7", features = ["scrt", "ensemble"] }
...
in order to implement it. Fadroma provides a convenient macro that will implement this
boilerplate for you. First, we do this for the auction contract:
...
We then could use the same macro to implement `ContractHarness` for the factory contract:

```rust
contract_harness! {
    Factory, 
    init: factory::instantiate,
    execute: factory::execute,
    query: factory::query,
    reply: factory::Contract::reply
}
```

But in order to understand what `contract_harness!` is generating we will do it by hand instead:
...
```
The first argument here is the struct name the we want generated and other parameters are
the entry point functions that we want to use.
...
the already familiar `ContractCode` struct that we can now use to create
